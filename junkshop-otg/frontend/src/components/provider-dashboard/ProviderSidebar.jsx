...
export default function ProviderSidebar({ activeTab, onNavigate, user }) {
    return (
...
export function ProviderMobileNav({ activeTab, onNavigate }) {
    ...
    return (
        <>
            ...
            {showMore && (
                <div className="lg:hidden fixed bottom-[4.25rem] left-3 right-3 z-50 rounded-2xl border border-zinc-200 bg-white p-2 shadow-xl">
                    {moreTabs.map((tab) => {
                        const Icon = tab.icon;
                        const isActive = activeTab === tab.id;
            ...
            <nav className="lg:hidden fixed bottom-0 left-0 w-full flex justify-around items-center px-1 py-2.5 bg-white border-t border-zinc-200 shadow-[0_-4px_12px_rgba(141,170,145,0.15)] z-50 rounded-t-2xl safe-area-pb">
                {mobileTabs.map((tab) => {
                    const Icon = tab.icon;
                    const isActive = activeTab === tab.id;
                    ...
                    return (
                        <button
                            ...
                            type="button"
                            onClick={() => handleNavigate(tab.id)}
                            className={`flex flex-col items-center justify-center min-w-[4rem] px-2 py-1.5 rounded-xl active:scale-95 transition-transform ${isActive
                                ? "bg-emerald-100 text-emerald-800"
                                : "text-zinc-500 hover:text-emerald-600"
                                }`}
                        >
                            <Icon size={20} />
                            <span className="text-[10px] font-medium mt-0.5 truncate max-w-[4.5rem]">
                                {tab.label}
                            </span>
                        </button>
                    );
                ...
                <button
                    ...
                    className={`flex flex-col items-center justify-center min-w-[4rem] px-2 py-1.5 rounded-xl active:scale-95 transition-transform ${moreActive || showMore
                        ? "bg-emerald-100 text-emerald-800"
                        : "text-zinc-500 hover:text-emerald-600"
                        }`}
                >
